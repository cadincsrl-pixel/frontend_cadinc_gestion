...
- **«Sale de la cuenta» se completa sola** (`20261009e`, dueño 27/09). Si nadie elige, `_pagos_emitir_orden` le pone a la OP la de `_pagos_cuenta_origen_sugerida`: efectivo → Caja; tarjeta → la última del proveedor; cheque/e-cheq propio → la cuenta del banco del cheque (la más usada si hay varias); transferencia, débito y cheques de tercero → la última usada con ese proveedor y forma, si no la más usada con esa forma; «otro» → ninguna. La pantalla la muestra como «Automática: X» (`POST /api/pagos/cuentas-origen/sugerida`, sin auditoría). Lo elegido a mano manda. Las 50 OP que estaban sin cuenta pasaron a Banco Galicia (efectivo → Caja).
- **Débito automático: se paga solo** (`20261009a`). El proveedor tiene forma habitual `debito_automatico` y una cuenta de débito (`pagos_proveedores.debito_cuenta_id`, en la ficha «Cuenta que le debita el banco»). Cuando una factura suya queda **imputada**, un constraint trigger diferido (`trg_pagos_debito_automatico` → `_pagos_debito_automatico`) hace tres cosas:
  1. La aprueba.
  2. Le aplica el crédito de sus notas de crédito aprobadas, porque el banco debita factura − NC.
  3. Paga el resto con una OP `debito_automatico` desde esa cuenta, con la fecha de la factura.

  Las NC de esos proveedores se aprueban solas y quedan como crédito. La doble firma no aplica, porque la plata ya la movió el banco: el control es la conciliación. Quedan afuera `pago_a_reconstruir`, `paga_cliente` y las observadas. Casos: Sancor (56) y La Segunda (49) → Banco Galicia.
...
- **Obra archivada no oculta nada en Compras, Ventas ni Contabilidad** (dueño, 27/09: «siempre se tiene que mostrar todo»; `20261008e`). Archivar es operativo (Tarja, Pedidos y Stock): las facturas, el resumen de Compras, los selectores de centro de costo (con grupo «Archivadas»), los habituales del proveedor y las obras de un cliente de Ventas las muestran siempre. El parámetro `archivadas` de `GET /facturas` quedó sin efecto. Caso: FA 25-25456 de Silva (CC-019 Hipódromo) no aparecía en pendientes.
...
- **Códigos del F.931 y jubilados** (`20261006a`, con el F.931 08/2026 de CADINC): condición/actividad/modalidad van por convenio (UOCRA 5/003/24, UECARA y Camioneros 1/049/8) y el legajo puede pisarlos (`f931_*`; la vista trae `f931_*_efectiva`). Localidad = parámetro `f931_localidad` (84). `sueldos_legajos.jubilado` saca los conceptos `excluye_jubilados` (INSSJP, obra social, contribución general) y pone los `solo_jubilados` (`contrib_ss_jubilado`, 10,77 % con detracción). ART = 6,53 % + $3.305 por CUIL, sacados del F.931. En el reg. 04 condición/modalidad/siniestrado van como texto a la izquierda (planilla de ARCA), actividad y situación con ceros.
- **Criterios del contador (27/09/2026)**: mensualizados y SAC con **mes de 30 días siempre** (`dias30`: febrero y meses de 31 = 30, semestre = 180; el mes del egreso entra por los días trabajados); sin SAC sobre vacaciones no gozadas; indemnización a mano según convenio y causa; contribución UOCRA **«tarea diferencial» 5 %** (`contrib_tarea_dif`, va al F.931, no a jubilados). Preguntas y respuestas: páginas https://claude.ai/artifact/TfbQLPAhxShdgQpNfUTgap y https://claude.ai/artifact/GbEhki1jH6rzpG9ckoDzMy (colección `respuestas`).
- **A confirmar con el contador** (al 27/09): base mínima de aportes (dijo «aplicá el mínimo»; falta el valor y si es solo obra social), cuentas de sindicato / fondo de cese / otros a pagar (las pasa él), códigos ARCA de viáticos y demás, y los valores marcados `a_confirmar`.
