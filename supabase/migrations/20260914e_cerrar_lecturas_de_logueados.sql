-- ⚠️ NO APLICADA TODAVÍA (al 2026-09-13). Escrita y probada con rollback contra la
-- base real, esperando que el user la corra y pruebe la app. Antes de aplicarla,
-- releer el inventario de accesos directos de abajo: si el frontend sumó alguna
-- lectura directa nueva desde entonces, hay que agregarla a los `grant`.
--   Verificar con:  grep -rE "\.from\(|\.rpc\(" src/ | grep -v Array.from
-- Prueba manual después de aplicar: entrar a la pantalla de ropa y cambiar de módulo.
--
