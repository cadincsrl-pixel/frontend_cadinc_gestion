...
import {
  useFlotaDocumentos,
  useUploadFlotaDocumento,
  useUpdateFlotaDocumento,
  useDeleteFlotaDocumento,
  fetchFlotaDocSignedUrl,
...
const TIPOS: { key: FlotaDocTipo; label: string; icon: string; venceObligatorio?: boolean }[] = [
  { key: 'titulo',         label: 'Título',           icon: '📜' },
  { key: 'tarjeta_verde',  label: 'Tarjeta verde',    icon: '🪪' },
  { key: 'vtv',            label: 'VTV',              icon: '🔍', venceObligatorio: true },
  { key: 'rto',            label: 'RTO',              icon: '🛠', venceObligatorio: true },
  { key: 'poliza_seguro',  label: 'Póliza de seguro', icon: '🛡', venceObligatorio: true },
  { key: 'patente',        label: 'Patente',          icon: '💳', venceObligatorio: true },
  { key: 'oblea',          label: 'Oblea GNC',        icon: '⚙️', venceObligatorio: true },
...
export function FlotaDocumentosSection({ vehiculoId }: Props) {
  const toast = useToast()
  const { puedeCrear, puedeEditar, puedeEliminar } = usePermisos('flota')

  const { data: docs = [], isLoading } = useFlotaDocumentos(vehiculoId)
  const { mutate: uploadDoc, isPending: uploading } = useUploadFlotaDocumento()
  const { mutate: updateDoc } = useUpdateFlotaDocumento()
  const { mutate: deleteDoc } = useDeleteFlotaDocumento()

  const fileInputs = useRef<Record<FlotaDocTipo, HTMLInputElement | null>>(
    Object.fromEntries(TIPOS.map(t => [t.key, null])) as Record<FlotaDocTipo, HTMLInputElement | null>,
  )
  const [pendingTipo, setPendingTipo] = useState<FlotaDocTipo | null>(null)
  const [pendingVence, setPendingVence] = useState<Record<FlotaDocTipo, string>>(
    Object.fromEntries(TIPOS.map(t => [t.key, ''])) as Record<FlotaDocTipo, string>,
  )

  function handleFileChange(tipo: FlotaDocTipo, e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setPendingTipo(tipo)
    uploadDoc(
      { vehiculoId, file, tipo, vence_el: venceVal },
      {
        onSuccess: () => {
  ...
  function handleEditarVence(doc: FlotaDocumento) {
    ...
    updateDoc(
      { vehiculoId, docId: doc.id, vence_el: trimmed || null },
      {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-bold text-azul uppercase tracking-wider">📂 Papeles del vehículo</h3>
