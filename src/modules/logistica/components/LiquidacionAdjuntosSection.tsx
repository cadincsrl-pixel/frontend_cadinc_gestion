...
import {
  useLiquidacionAdjuntos, useUploadLiquidacionAdjunto, useDeleteLiquidacionAdjunto,
  fetchLiquidacionAdjSignedUrl,
...
const TIPOS: { key: LiquidacionAdjuntoTipo; label: string; icon: string }[] = [
  { key: 'comprobante_pago', label: 'Comprobante de pago',  icon: '💰' },
  { key: 'recibo_firmado',   label: 'Recibo firmado',       icon: '✍️' },
]
...
export function LiquidacionAdjuntosSection({ liqId }: Props) {
  const toast = useToast()
  const { puedeCrear, puedeEliminar } = usePermisos('logistica')

  const { data: adjuntos = [], isLoading } = useLiquidacionAdjuntos(liqId)
  const { mutate: uploadAdj, isPending: uploading } = useUploadLiquidacionAdjunto()
  const { mutate: deleteAdj } = useDeleteLiquidacionAdjunto()

  const fileInputs = useRef<Record<LiquidacionAdjuntoTipo, HTMLInputElement | null>>({
    comprobante_pago: null, recibo_firmado: null,
  })
  const [pendingTipo, setPendingTipo] = useState<LiquidacionAdjuntoTipo | null>(null)

  function handleFileChange(tipo: LiquidacionAdjuntoTipo, e: React.ChangeEvent<HTMLInputElement>) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-bold text-azul uppercase tracking-wider">📂 Adjuntos de la liquidación</h3>
