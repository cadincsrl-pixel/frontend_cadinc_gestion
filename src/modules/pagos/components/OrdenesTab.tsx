      ...
      {lista.isLoading && !lista.data ? (
        ...
        <div className="bg-white rounded-card shadow-card overflow-hidden">
          ...
          <div className="md:hidden divide-y divide-gris">
            {items.map(o => (
              <button key={o.id} type="button" onClick={() => setDetalleId(o.id)}
                      className={`w-full text-left p-3 ${o.estado === 'anulada' ? 'opacity-60' : ''}`}>
...
function DetalleOrden({ id, onClose, puedeAnular, puedeSubir, verPii, toast }: {
  id: number; onClose: () => void; puedeAnular: boolean; puedeSubir: boolean; verPii: boolean
