...
export function HistorialPrecios({ material, onClose, onUsarPrecio, ocupado }: Props) {
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gris">
                  {compras.map((c: MaterialCompra) => {
                    ...
                    return (
                      <tr key={c.item_id} className="hover:bg-gris/20">
                        ...
                        <td className={`px-3 py-1.5 text-right font-mono font-bold tabular-nums ${precio === ref ? 'text-azul' : ''}`} title={precio === ref ? 'Es el precio de referencia actual' : undefined}>{fmtM(precio)}</td>
                        ...
                        {onUsarPrecio && (
                          <td className="px-3 py-1.5 text-right">
                            {precio !== ref && <button type="button" disabled={ocupado} onClick={() => onUsarPrecio(precio)} className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-azul-light text-azul hover:opacity-80" title="Copiar este precio al precio de referencia">usar</button>}
                          </td>
                        )}
                      </tr>
                    )
